...
def check(config_path, train_path, test_path, num_clients, num_classes, niid=False,
    ...
    print('check alpha: {}'.format(alpha))
...
def separate_data(data, num_clients, num_classes, niid=False, balance=False, partition=None, class_per_client=2, alpha=0.1):
    ...
    print('eparate_data alpha: {}'.format(alpha))
...
def split_data2(X, y):
    ...
    for i in range(len(y)):
        ...
        if min(count) > 1:
            ...
            X_train, X_test, y_train, y_test = train_test_split(
                X[i], y[i], train_size=train_size, shuffle=True)
        tmp_X = np.concatenate((X_train, X_test), axis=0)
        tmp_y = np.concatenate((y_train, y_test), axis=0)
        train_data.append({'x': tmp_X, 'y': tmp_y})
        num_samples['train'].append(len(y_train))
    print('total number of train samples: {}'.format(
        sum(num_samples['train'])))
...
def save_file2(config_path, train_path, test_path, train_data, num_clients, num_classes, statistic, niid=False, balance=True, partition=None, alpha=0.1):
    print('save file2 alpha: {}'.format(alpha))
    config = {
...
def save_file(config_path, train_path, test_path, train_data, test_data, num_clients,
    ...
    print('save file alpha: {}'.format(alpha))
    config = {
        'num_clients': num_clients,
        'num_classes': num_classes,
        'non_iid': niid,
