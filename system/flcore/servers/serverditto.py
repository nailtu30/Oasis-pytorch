...
class Ditto(Server):
    ...
    def train(self):
        ...
        for i in range(self.global_rounds+1):
            ...
            print(D_losses)
            print(G_losses)
            with open(os.path.join(self.results_dir, 'pD_loss.csv'), 'a') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(pD_losses)
            with open(os.path.join(self.results_dir, 'pG_loss.csv'), 'a') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(pG_losses)
