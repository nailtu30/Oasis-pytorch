import time
from flcore.clients.clientsim2 import clientSim
...
class FedSim2(Server):
    ...
    def create_similarity_table(self):
        ...
        print('find best K')
        # Ks = np.arange(self.num_clients - 1) + 1
        # for Metric in METRICS:
        #     print(Metric)
        Ks = np.arange(1, 50) + 1
        # min_total_cluster_distance = np.inf
        max_silhouette_score = -np.inf
        best_K = self.num_clients
        Metric = 'correlation'
        for K in Ks:
            print('======K {}======='.format(K))
            cluster_size = K
            ...
            tmp_groups = [[] for _ in range(cluster_size)]
            tmp_cluster = [0] * self.num_clients
            for id, (group_id, distance) in enumerate(cluster):
                group_id = group_id.astype(int)
                tmp_groups[group_id].append(id)
                tmp_cluster[id] = group_id
            for group_idx, client_ids in enumerate(tmp_groups):
                data_size = 0
            ...
            print(tmp_cluster)
            score = silhouette_score(representations, tmp_cluster, metric=Metric)
            print('score: {}'.format(score))
            if score > max_silhouette_score:
                max_silhouette_score = score
                print('max_silhouette_score: {}'.format(max_silhouette_score))
                self.cluster = tmp_cluster
                self.groups = tmp_groups
                best_K = K
            # if total_cluster_distance < min_total_cluster_distance:
            #     min_total_cluster_distance = total_cluster_distance
            #     self.groups = groups
            #     best_K = K
            #     print('best K: {}, min_total_cluster_distance: {}'.format(best_K, min_total_cluster_distance))
        # self.groups = groups
        # print(self.clutser)
        print('='*50)
        print('Metric: {}, best K: {}'.format(Metric, best_K))
        print(self.groups)
        self.cluser_size = best_K
        self.model_repository = [[copy.deepcopy(self.global_model_D), copy.deepcopy(
        self.global_model_G)] for _ in range(self.cluser_size)]
        # print(self.model_repository)
        self.group_selected_times = [0] * self.cluser_size
        ...
        # with open('metric_k.csv', 'a') as csvfile:
        #     writer = csv.writer(csvfile)
        #     writer.writerow([Metric, best_K])
