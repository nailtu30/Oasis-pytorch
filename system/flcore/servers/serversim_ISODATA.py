import time
from flcore.clients.clientsim_ISODATA import clientSim
...
from utils.ISODATA import ISODATA
import copy
from sklearn.decomposition import FastICA


class FedSimISODATA(Server):
    ...
    def create_similarity_table(self):
        ...
        isoData = ISODATA(data=representations, designCenterNum=10, LeastSampleNum=20,
                          StdThred=0.01, LeastCenterDist=1000, iterationNum=200)
        isoData.train()
        cluster = isoData.label
        groups = [[] for _ in range(cluster_size)]
        self.cluster = []
        for id, group_id in enumerate(cluster):
