...
class clientAVG(Client):
    ...
    def max_norm(self, model, max_val=3, eps=1e-8):
        for name, param in model.named_parameters():
            if 'bias' not in name:
                norm = param.norm(2, dim=0, keepdim=True)
                norm = norm.clamp(min=max_val / 2)
    ...
    def D_train(self, x):
        ...
        D_loss = D_loss + orth_loss

        D_loss.backward()

        self.D_optimizer.step()

        # self.max_norm(self.model_D)
    ...
    def G_train(self, x):
        ...
        # G_loss = G_loss + orth_loss

        # gradient backprop & optimize ONLY G's parameters
        G_loss.backward()
        self.G_optimizer.step()

        self.max_norm(self.model_G)
