...
class clientAVG(Client):
    ...
    def D_train(self, x):
        ...
        D_loss = D_loss + l2_lambda * sum(torch.linalg.norm((p - op).flatten(), 2)
                                          ** 2 for p, op in zip(self.model_D.parameters(), self.old_D.parameters()))
        # D_loss = D_loss + \
        #     l2_lambda * sum(torch.linalg.norm(p, 1)
    ...
    def G_train(self, x):
        ...
        G_loss = G_loss + l2_lambda * sum(torch.linalg.norm((p - op).flatten(), 2)
                                          ** 2 for p, op in zip(self.model_G.parameters(), self.old_G.parameters()))
    ...
    def train(self):
        ...
        self.old_D = copy.deepcopy(self.model_D)
        self.old_G = copy.deepcopy(self.model_G)
